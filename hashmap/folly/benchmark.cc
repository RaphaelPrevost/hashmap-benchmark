...
// Mode parsing
static bool is_mode(const char* arg, const char* mode)
{
    return std::strcmp(arg, mode) == 0;
}
...
int main(int argc, char** argv)
{
    ...
    if (is_mode(mode, "miss")) {
        for (uintptr_t i = 1; i <= count; ++i) {
            ...
            len = static_cast<std::size_t>(n);
            if (h.find(Key(key, len)) != h.end()) return EXIT_FAILURE;
        }
